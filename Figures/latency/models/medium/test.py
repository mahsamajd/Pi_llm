...
def extract_metrics(file_content):
    ...
    patterns = {
        ...
        'Overall Time': r'Overall Time: (\d+\.\d+)',
        'Other Data Processing Time': r'Other Data Processing Time: (\d+\.\d+)'
    }
...
def get_model_name(filename):
    return filename.split("en_")[-1].split(".txt")[0]
...
def save_to_csv(averages, output_file):
    with open(output_file, 'w', newline='') as csvfile:
        fieldnames = ['Model', 'Avg Transcription Time', 'Avg Words per Second', 'Avg Inference Time', 'Avg Tokens per Second', 'Avg Overall Time', 'Avg Other Data Processing Time']
        ...
        for model, metrics in averages.items():
            writer.writerow({
                'Model': model,
                'Avg Transcription Time': f"{metrics['Transcription Time']:.6f}",
                'Avg Words per Second': f"{metrics['Words per Second']:.6f}",
                'Avg Inference Time': f"{metrics['Inference Time']:.6f}",
                'Avg Tokens per Second': f"{metrics['Tokens per Second']:.6f}",
                'Avg Overall Time': f"{metrics['Overall Time']:.6f}",
                'Avg Other Data Processing Time': f"{metrics['Other Data Processing Time']:.6f}"
            })

# Main execution
directory = '.'  # Current directory, change if needed
output_file = 'model_averages_medium.csv'
